...
namespace TravelWebsite.Business.Services.PlaceService
{
    ...
    public class PropertyService : IPropertyService
    {
        ...
        {
            var result = await _context.Properties.Include(item => item.User).Include(item => item.Bookings).ThenInclude(item => item.User).Include(item => item.City).Include(item => item.Type).Include(item => item.Images).FirstOrDefaultAsync(item => item.Id == id);
